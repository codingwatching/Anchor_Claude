...
function anchor:font_get_text_width(name, text)
  return self.fonts[name].source:getWidth(tostring(text))
end
...
--[[
  Loads and adds a tiled map object to the engine object.
  Added maps can be accessed via an.tiled_maps.name.
  Example:
    an:tiled_map('map_1', 'assets/map_1.lua')
    an.tiled_maps.map_1:get_layer_vertices('solids') -> returns solid vertices
--]]
function anchor:tiled_map(name, filename)
  self.tiled_maps[name] = object():_tiled_map(filename)
end
