...
--[[
  Returns a table that was initially laid out with points (.x, .y attributes as a table for each point) to a table where each point is laid out as flat vertices (x, y attributes are i, i+1).
  TODO: examples
--]]
function math.get_flat_vertices_from_points(points)
  local new_vertices = {}
  for _, p in ipairs(points) do
    table.insert(new_vertices, p.x)
    table.insert(new_vertices, p.y)
  end
  return new_vertices
end

--[[
  Returns n points horizontally centered around position x, y, with the given spacing between each other.
  These points are manually defined by me such that they look nice, and should be used in various UI contexts where you need things centered horizontally around a point.
  The values are returned as a table of points, each point being a table with .x and .y attributes.
  TODO: examples
--]]
function math.get_horizontally_centered_points(x, y, n, spacing)
  local points = {}
  if n % 2 == 0 then
    local x = x - (n/2)*spacing + spacing/2
    for i = 1, n do
      table.insert(points, {x = x + (i-1)*spacing, y = y})
    end
  elseif n % 2 == 1 then
    if n == 1 then
      table.insert(points, {x = x, y = y})
    else
      for i = math.ceil(-n/2), math.floor(n/2), 1 do
        table.insert(points, {x = x + i*spacing, y = y})
      end
    end
  end
  return points
...
--[[
  Exactly the same as math.get_horizontally_centered_points except the points are vertical instead.
--]]
function math.get_vertically_centered_points(x, y, n, spacing)
  local points = {}
  if n % 2 == 0 then
    local y = y - (n/2)*spacing + spacing/2
    for i = 1, n do
      table.insert(points, {x = x, y = y + (i-1)*spacing})
    end
  elseif n % 2 == 1 then
    if n == 1 then
      table.insert(points, {x = x, y = y})
      ...
      for i = math.ceil(-n/2), math.floor(n/2), 1 do
        table.insert(points, {x = x, y = y + i*spacing})
      end
    end
  end
  return points
...
--[[
  Returns a table that was initially laid out with flat vertices (x, y attributes are i, i+1) to a table where each point is its own table with .x and .y attributes.
  TODO: examples
--]]
function math.get_points_from_flat_vertices(vertices)
  local new_vertices = {}
  for i = 1, #vertices, 2 do
    local x, y = vertices[i], vertices[i+1]
    table.insert(new_vertices, {x = x, y = y})
  end
  return new_vertices
end
...
--[[
  Translates the point by the given x, y amount and then rotates it.
  This is equivalent to calling push_trs, except you get the resulting point values.
  TODO: examples
--]]
function math.point_trs(x, y, tx, ty, r)
