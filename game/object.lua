do
	...
	local _base_0 = {
		add = function(self, child)
			do
				local _obj_0 = self.children
				_obj_0[#_obj_0 + 1] = child
			end
			child.parent = self
			if child.name then
				if self[child.name] then
					self[child.name]:kill()
				end
		...
		kill = function(self, tag)
			if tag then
				if self.tags[tag] then
					self:kill()
				else
					local _list_0 = self:all(tag)
				...
				if self.linked_from then
					local _list_0 = self.linked_from
					for _index_0 = 1, #_list_0 do
						local link = _list_0[_index_0]
						if link.source.dead then
							goto _continue_0
						end
		...
		is = function(self, name_or_tag)
			return self.name == name_or_tag or self.tags[name_or_tag]
		end,
		link = function(self, target, callback)
			if not self.links then
				self.links = { }
			end
			do
				local _obj_0 = self.links
				_obj_0[#_obj_0 + 1] = {
					target = target,
					callback = callback
				}
			end
			if not target.linked_from then
				target.linked_from = { }
			end
			do
				local _obj_0 = target.linked_from
				_obj_0[#_obj_0 + 1] = {
					source = self,
					callback = callback
				}
		...
		build = function(self, build_function)
			build_function(self)
			return self
		end,
		flow_to = function(self, parent)
			parent:add(self)
			return self
		end,
		early_action = function(self, name_or_fn, fn)
			if not self.early_actions then
				self.early_actions = { }
			end
			if not self.early_action_names then
				self.early_action_names = { }
			end
			if type(name_or_fn) == 'string' then
				...
				do
					local _obj_0 = self.early_actions
					_obj_0[#_obj_0 + 1] = fn
				end
				do
					local _obj_0 = self.early_action_names
					_obj_0[#_obj_0 + 1] = name
				end
				self[name] = fn
			else
				do
					local _obj_0 = self.early_actions
					_obj_0[#_obj_0 + 1] = name_or_fn
				end
				local _obj_0 = self.early_action_names
				_obj_0[#_obj_0 + 1] = false
			end
			return self
		end,
		action = function(self, name_or_fn, fn)
			if not self.actions then
				self.actions = { }
			end
			if not self.action_names then
				self.action_names = { }
			end
			if type(name_or_fn) == 'string' then
				local name = name_or_fn
				...
				do
					local _obj_0 = self.actions
					_obj_0[#_obj_0 + 1] = fn
				end
				do
					local _obj_0 = self.action_names
					_obj_0[#_obj_0 + 1] = name
				end
				self[name] = fn
			else
		...
		late_action = function(self, name_or_fn, fn)
			if not self.late_actions then
				self.late_actions = { }
			end
			if not self.late_action_names then
				self.late_action_names = { }
			end
			if type(name_or_fn) == 'string' then
				local name = name_or_fn
				...
				do
					local _obj_0 = self.late_actions
					_obj_0[#_obj_0 + 1] = fn
				end
				do
					local _obj_0 = self.late_action_names
					_obj_0[#_obj_0 + 1] = name
				end
				self[name] = fn
			else
		...
		E = function(self, name_or_fn, fn)
			return self:early_action(name_or_fn, fn)
		end,
		X = function(self, name_or_fn, fn)
			return self:action(name_or_fn, fn)
		end,
		L = function(self, name_or_fn, fn)
			return self:late_action(name_or_fn, fn)
		end,
		A = function(self, child)
			return self:add(child)
		end,
		K = function(self, target, callback)
			return self:link(target, callback)
		end,
		Y = function(self, properties)
			return self:set(properties)
		end,
		U = function(self, build_function)
			return self:build(build_function)
		end,
		F = function(self, parent)
			return self:flow_to(parent)
		end,
		_early_update = function(self, dt)
			if self.dead then
				return
			end
			...
			if self.early_actions then
				for i, fn in ipairs(self.early_actions) do
					if fn(self, dt) == true then
						if not self.early_actions_to_remove then
							self.early_actions_to_remove = { }
						end
						local _obj_0 = self.early_actions_to_remove
						_obj_0[#_obj_0 + 1] = i
					end
				end
			end
		...
		_update = function(self, dt)
			...
			if self.actions then
				for i, fn in ipairs(self.actions) do
					if fn(self, dt) == true then
						if not self.actions_to_remove then
							self.actions_to_remove = { }
						end
						local _obj_0 = self.actions_to_remove
						_obj_0[#_obj_0 + 1] = i
					end
				end
			end
		end,
		_late_update = function(self, dt)
			if self.dead then
				return
			end
			...
			if self.late_actions then
				for i, fn in ipairs(self.late_actions) do
					if fn(self, dt) == true then
						if not self.late_actions_to_remove then
							self.late_actions_to_remove = { }
						end
						local _obj_0 = self.late_actions_to_remove
						_obj_0[#_obj_0 + 1] = i
					end
				end
			end
		...
		cleanup = function(self)
			...
			for i = #objects, 1, -1 do
				...
				while j <= #parent.children do
					...
					if child.dead then
						if child.links then
							local _list_1 = child.links
							for _index_0 = 1, #_list_1 do
								local link = _list_1[_index_0]
						...
						if child.linked_from then
							local _list_1 = child.linked_from
							for _index_0 = 1, #_list_1 do
