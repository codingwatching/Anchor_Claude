...
--[[
  Follows the given path. Returns the force to be applied to the object.
  The path is a table containing multiple points, each being a table with .x and .y attributes.
  path_direction tells if we're going to follow this path by increasing or decreasing its index.
  point_seek_distance is the minimum distance required of this object for us to change to the next point.
  "self.collider_path_current_point_index" is the current point index for the path that's being followed. It isn't set by default and automatically looks for the closest point in the path to follow.
  This value should be reset manually once the current follow path use is done. If this isn't done, then the next time a path is to be followed, it will start from the index it last ended on.
  Example:
    -- Create a path with 8 points circling around an.w/2, an.h/2 with radius 50
    local path, r = {}, 0
    for i = 1, 8 do
      table.insert(path, {x = an.w/2 + 50*math.cos(r), y = an.h/2 + 50*math.sin(r)})
      r = r + math.pi/4
    end
    -- Then in some collider's update function
    local px, py = self:collider_follow_path(path, 1, 5, self.max_v)
--]]
function collider:collider_follow_path(path, path_direction, point_seek_distance, max_speed, max_force)
  if not self.collider_path_current_point_index then
    self:collider_find_closest_path_point(path, path_direction)
  end

  local p = path[self.collider_path_current_point_index]
  if math.distance(p.x, p.y, self.x, self.y) < point_seek_distance then
    self.collider_path_current_point_index = self.collider_path_current_point_index + (path_direction or 1)
    if self.collider_path_current_point_index < 1 then self.collider_path_current_point_index = #path end
    if self.collider_path_current_point_index > #path then self.collider_path_current_point_index = 1 end
  else
    return self:collider_seek(p.x, p.y, max_speed, max_force or 1000)
  end
  return 0, 0
end

--[[
  Finds the closest path point to this collider.
  This is used internally by "collider_follow_path", but it might also be useful to the user in certain circumstances.
  This function both sets "self.collider_path_current_point_index" to the index of the closest point (this index is for the "path" table), and also returns the point itself.
--]]
function collider:collider_find_closest_path_point(path, path_direction)
  local min_d, min_i = 100000, 0
  for i, p in ipairs(path) do
    local d = math.distance(p.x, p.y, self.x, self.y)
    if d < min_d then
      min_d = d
      min_i = i
    end
  end
  self.collider_path_current_point_index = min_i
  return path[self.collider_path_current_point_index]
end
